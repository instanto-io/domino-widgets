...
public final class ColumnsGroupsExamples implements org.dominokit.domino.ui.style.DominoCss {
  ...
  private void groupColumns() {
    ...
    tableConfig
        .addColumn(
            ColumnConfig.<Contact>create("personalInfo", "Personal info")
                .addColumn(
                    ColumnConfig.<Contact>create("identity", "Identity")
                        .addColumn(
                            ColumnConfig.<Contact>create("id", "#")
                                .setTextAlign(CellTextAlign.RIGHT)
                                .setCellRenderer(
                                    cell ->
                                        text(cell.getTableRow().getRecord().getIndex() + 1 + "")))
                        .addColumn(
                            ColumnConfig.<Contact>create("firstName", "First name")
                                .setCellRenderer(
                                    cell -> text(cell.getTableRow().getRecord().getName()))))
                .addColumn(
        ...
        .addColumn(
            ColumnConfig.<Contact>create("contact", "Contact")
                .addColumn(
    ...
    element.appendChild(
        Card.create("GROUP COLUMNS", "Group columns together for resize and pinning.")
            .setCollapsible(true)
            .appendChild(table)
            .element());
  ...
  private void groupPinResizeColumns() {
    ...
    tableConfig
            ...
            ColumnConfig.<Contact>create("identity", "Identity")
                .addColumn(
                    ColumnConfig.<Contact>create("id", "#")
                        ...
                        .setTextAlign(CellTextAlign.RIGHT)
                        .setCellRenderer(
                            cell -> text(cell.getTableRow().getRecord().getIndex() + 1 + "")))
                .addColumn(
                    ColumnConfig.<Contact>create("firstName", "First name")
                        .setWidth("200px")
                        .minWidth("100px")
        ...
        .addPlugin(
            new PinColumnsPlugin<Contact>()
                .configure(config -> config.setShowPinIcon(true).setShowPinMenu(true)))
    ...
    element.appendChild(
        Card.create(
                "GROUP, PIN, RESIZE COLUMNS", "Column groups works with ping and resize plugins.")
            .setCollapsible(true)
            .appendChild(table)
            .element());
