...
Running Club now lives under `demos/running-club` in Verrai, commit `e76cace`, included by Verrai's `demo` Maven profile. The complete application was carried over from Sarto commit `0ca3268418d701bb414ba3e254b074ba7d64b6ab`, preserving its DominoKit UI, five services, both hosts and club configurations. Full TeaVM client compilation and Maven verification pass, including seven Chrome tests using the actual events page, generated Verrai template binder, real Caller and original standalone calendar.
...
Earlier baseline failures were resolved by refreshing inconsistent framework snapshots and rebuilding the committed Verrai persistence artifact at `1f1cb93`; no Verrai source changes were needed. An older cached CDI POM still emits an obsolete-async dependency warning without failing the verified client build. The separate legacy Verrai wrapper module remains for its existing contract tests.
