...
public final class TreeGridLazyPluginExamples implements org.dominokit.domino.ui.style.DominoCss {
  ...
  private void lazyTreeGridFullParentSpan() {
    ...
    tableConfig
        ...
        .addPlugin(
            new RowMarkerPlugin<>(
                tableCellInfo ->
                    ContactUiUtils.getBalanceColor(tableCellInfo.getRecord())
                        .color()
                        .getContextColor()))
        .addPlugin(
            new TreeGridPlugin<Contact>()
                .configure(
                    config -> {
                      config
                          .setIndentColumnElementSupplier(
                              tableRow ->
                                  p(tableRow.getRecord().getName()).addCss(dui_m_0).element())
                          .setParentRowCellsSupplier(
                              (dataTable, tableRow) -> {
                                HTMLTableCellElement cellElement =
                                    td().setAttribute("colspan", "8").element();
                                RowCell<Contact> rowCell =
                                    new RowCell<>(
                                        new org.dominokit.domino.ui.datatable.RowCellInfo<>(
                                            tableRow, cellElement),
                                        dataTable.getTableConfig().getColumnByName("id"));
    ...
    LocalTreeDataStore<Contact> localListDataStore =
        new LocalTreeDataStore<>(
            (parent, itemsConsumer) -> {
              // If the friend is in the max depth it will automatically set has children to false,
              // check ContactsProvider.instance.addFriends method
    ...
    element.appendChild(
        Card.create(
                "LAZY TREE GRID PLUGIN - Full PARENT SPAN",
                "Render records in tree style with expand and collapse features, ")
            .setCollapsible(true)
            .appendChild(table)
            .element());
  ...
  private void lazyTreeGridParentColumns() {
    ...
    tableConfig
        ...
        .onUtilityColumn(
            utilityColumn -> {
              utilityColumn.setSortable(true, "id").setTitle("First name");
            })
        .setMultiSelect(true)
        .addPlugin(new SortPlugin<>())
        .addPlugin(new SelectionPlugin<>())
    ...
    element.appendChild(
        Card.create(
                "LAZY TREE GRID PLUGIN - PARENT WITH COLUMNS",
                "Render records in tree style with expand and collapse features, sub records wont"
                    + " be loaded until the row is expanded.")
            .setCollapsible(true)
            .appendChild(table)
            .element());
  ...
  private void largeLazyTreeGridParentColumns() {
    ...
    LocalTreeDataStore<Contact> localListDataStore =
        new LocalTreeDataStore<>(
            (parent, itemsConsumer) -> {
              ContactsProvider.instance.addFriends(
                  parent, leafCounts.getValue(), 1, depth.getValue());
              itemsConsumer.accept(Optional.ofNullable(parent.getFriends()));
            });
    ...
    updateListButton.addClickListener(
        evt ->
