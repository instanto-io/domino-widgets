...
public final class GridLayoutExamples implements org.dominokit.domino.ui.style.DominoCss {
  ...
  private void initLayoutSamples() {
    GridLayout gridLayout = GridLayout.create().setHeight("500px");
    ...
    Slider headerSlider = Slider.create(6, 0, 1).withThumb().setStep(1);
    headerSlider.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setHeaderSpan(SectionSpan.valueOf("_" + newValue));
        });

    Slider leftSlider = Slider.create(6, 0, 3).withThumb().setStep(1);
    CheckBox leftSpanUpCheck = CheckBox.create("Span Up");
    CheckBox leftSpanDownCheck = CheckBox.create("Span Down");
    leftSpanUpCheck.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setLeftSpan(
              SectionSpan.valueOf("_" + leftSlider.getValue()),
              newValue,
              leftSpanDownCheck.getValue());
        });

    leftSpanDownCheck.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setLeftSpan(
              SectionSpan.valueOf("_" + leftSlider.getValue()),
              leftSpanUpCheck.getValue(),
              newValue);
        });
    leftSlider.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setLeftSpan(
              SectionSpan.valueOf("_" + newValue),
              leftSpanUpCheck.getValue(),
              leftSpanDownCheck.getValue());
        });

    Slider rightSlider = Slider.create(6, 0, 3).withThumb().setStep(1);
    CheckBox rightSpanUpCheck = CheckBox.create("Span Up");
    CheckBox rightSpanDownCheck = CheckBox.create("Span Down");
    rightSpanUpCheck.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setRightSpan(
              SectionSpan.valueOf("_" + rightSlider.getValue()),
              newValue,
              rightSpanDownCheck.getValue());
        });

    rightSpanDownCheck.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setRightSpan(
              SectionSpan.valueOf("_" + rightSlider.getValue()),
              rightSpanUpCheck.getValue(),
              newValue);
        });
    ...
    Slider footerSlider = Slider.create(6, 0, 1).withThumb().setStep(1);
    footerSlider.addChangeListener(
        (oldValue, newValue) -> {
          gridLayout.setFooterSpan(SectionSpan.valueOf("_" + newValue));
        });

    Slider gapSlider =
        Slider.create(10, 0, 0)
            .withThumb()
            .setStep(1)
        ...
        Card.create()
            .appendChild(
                Row.create()
                    ...
                    .appendChild(
                        Column.span2().appendChild(gapSlider).appendChild(h(5).textContent("Gap"))))
    ...
    element.appendChild(
        Card.create("USAGE")
            .appendChild(
                BlockHeader.create(
                        "Basic",
                        "Grid layout is a 12 columns grid based layout with a content section and"
                            + " another 4 optional sections: Header. Footer, Left and Right. ")
                    .appendChild(
                        a().setAttribute(
            ...
            .appendChild(
                h(5).textContent(
                        "The GridLayout is a single div element that can hold up to 5 other divs"
                            + " for each section"))
            .appendChild(br())
            .appendChild(h(5).textContent("The Content"))
            .appendChild(
                h(6).textContent(
                        "Content section is the only section in the GridLayout by default, it"
                            + " covers all 12 rows and columns of the grid by default."))
            .appendChild(br())
            ...
            .appendChild(
                h(6).textContent("You can span a section to cover up to 6 rows or columns."))
            .appendChild(br())
            ...
            .appendChild(
                h(6).textContent(
                        "You have to span a section to make the appended elements show up."))
            .appendChild(br())
