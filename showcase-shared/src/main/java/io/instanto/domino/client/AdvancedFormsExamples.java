...
public final class AdvancedFormsExamples implements org.dominokit.domino.ui.style.DominoCss {
  ...
  private void initFileUploadExample() {
    element.appendChild(
        Card.create("FILE UPLOAD - DRAG & DROP OR WITH CLICK & CHOOSE")
            .appendChild(BlockHeader.create("Manual upload"))
            .appendChild(
                FileUpload.create()
                    .setDecoration(
                        DefaultFileUploadDecoration.create()
                            .setIcon(Icons.upload().addCss(dui_font_size_24, dui_fg_accent))
                            .setTitle("Drop files here or click to upload.")
                            .setDescription(
                                "(This is just a demo upload. Selected files are not actually"
                                    + " uploaded)")
                            .addCss(dui_m_b_4))
                    .setAutoUpload(false)
                    .onAddFile(
                        fileItem -> {
                          fileItem
                              .addErrorHandler(
                                  request ->
                                      Notification.create("Failed to upload file")
                                          .addCss(dui_error)
                                          .show())
                              .addRemoveHandler(
                                  file ->
                                      Notification.create("File removed")
                                          .addCss(dui_warning)
                                          .show())
                              .addBeforeUploadHandler(
                                  (request, formData) ->
                                      Notification.create("Starting file upload")
                                          .addCss(dui_primary)
                                          .show())
                              .addCancelHandler(
                                  request ->
  ...
  private void initTagsInputExample() {
    ...
    element.appendChild(
        card.appendChild(
                BlockHeader.create("FREE TEXT TAGS", "Free text tags accept any text value"))
            .appendChild(
                Row.create()
                    .span6(
                        TagBox.create("Tag input", token -> token)
    ...
    OrderedSuggestionsStore<Person, Chip, TagOption<Person>> personsStore =
        OrderedSuggestionsStore.<Person, Chip, TagOption<Person>>create(
                person -> Optional.of(TagOption.create(person.name, person, person.name)))
            .setMissingEntryProvider(
                inputValue ->
                    Optional.of(TagOption.create(inputValue, new Person(inputValue), inputValue)))
            .setMissingValueProvider(
                person -> Optional.of(TagOption.create(person.name, person, person.name)))
    ...
    card.appendChild(
            BlockHeader.create("SELECT TAGS", "Select tags have store of objects to be selected"))
        .appendChild(
            Row.create()
                .span6(
                    TagBox.create("Friends", personsStore)
                        .withValue(Collections.singletonList(schroederColeman))
                        .setPlaceholder("Friend's name...")
                        .addChangeListener(
                            (oldValue, newValue) -> {
                              Notification.create("OLD[" + oldValue + "], NEW[" + newValue + "]")
                                  .show();
                            }))
                .span6(
                    TagBox.create("IP addresses", token -> token)
                            ...
                            (tagBox) -> {
                              ...
                              for (TagOption<String> v : selectedOptions) {
                                if (!new JsRegExp(IP_ADDRESS_REGEX).test(v.getValue())) {
                                  v.getComponent()
                                      .addCss(dui_border, dui_border_red, dui_border_solid);
        ...
        .appendChild(
            Row.create()
                .span6(
                    TagBox.create("Tag input: Readonly", token -> token)
                        .setPlaceholder("Type anything...")
                        .setRemovable(true)
                        .withValue(Arrays.asList("Tag A", "Tag B"))
                        .addChangeListener(
                        ...
                        .setReadOnly(true))
                .span6(
                    TagBox.create("Tag input: Disabled", token -> token)
                        .setPlaceholder("Type anything...")
                        .setRemovable(true)
                        .withValue(Arrays.asList("Tag A", "Tag B"))
                        .addChangeListener(
  ...
  private void initSuggestBoxExample() {
    LocalSuggestionsStore<String, SpanElement, SuggestOption<String>> localStore =
        LocalSuggestionsStore.<String, SpanElement, SuggestOption<String>>create()
            ...
            .setMissingValueProvider(
                missingValue -> Optional.of(SuggestOption.create(missingValue)));

    TextBox friendNameBox =
        TextBox.create("Your friend name")
            ...
            .apply(
                textBox -> {
                  textBox.appendChild(
                      PostfixAddOn.of(
                          Button.create("ADD FRIEND")
                              .addCss(dui_primary, dui_h_8, dui_leading_5)
                              .addClickListener(
                                  evt -> {
                                    localStore.addSuggestion(
                                        SuggestOption.create(textBox.getValue()));
                                    textBox.clear();
                                  })));
                });
    ...
    element.appendChild(
        card.appendChild(BlockHeader.create("Local suggestions"))
            .appendChild(
                Row.create()
                    .span6(friendNameBox)
                    .span6(
                        SuggestBox.create(localStore)
    ...
    SuggestionsStore<String, SpanElement, SuggestOption<String>> dynamicStore =
        new SuggestionsStore<String, SpanElement, SuggestOption<String>>() {

          @Override
          public void filter(
              String searchValue,
              SuggestionsHandler<String, SpanElement, SuggestOption<String>> suggestionsHandler) {
                    ...
                    json -> {
                      ...
                      for (int i = 0; i < randomNames.length; i++) {
                        ...
                        if (nameProperties
                            ...
                            .toLowerCase()
                            .contains(searchValue.toLowerCase())) {
                          SuggestOption<String> suggestItem =
                              SuggestOption.create(nameProperties.get("name"));
    ...
    card.appendChild(BlockHeader.create("Dynamic suggestions"))
        .appendChild(
            Row.create()
                .span6(
                    SuggestBox.create(dynamicStore)
                ...
                .span6(
                    MultiSuggestBox.create(dynamicStore)
