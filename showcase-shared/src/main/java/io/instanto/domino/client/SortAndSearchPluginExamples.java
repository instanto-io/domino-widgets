...
public final class SortAndSearchPluginExamples implements org.dominokit.domino.ui.style.DominoCss {
  ...
  private void sortAndSearch() {
    ...
    tableConfig
        ...
        .addColumn(
            ColumnConfig.<Contact>create("gender", "Gender")
                ...
                .setTextAlign(CellTextAlign.CENTER)
                .applyMeta(
                    ColumnFilterMeta.of(EnumHeaderFilter.<Contact, Gender>create(Gender.values()))))
        .addColumn(
    ...
    ColumnHeaderFilterPlugin<Contact> contactColumnHeaderFilterPlugin =
        ColumnHeaderFilterPlugin.<Contact>create();
    tableConfig
        .addPlugin(new SortPlugin<>())
        .addPlugin(
            new HeaderBarPlugin<Contact>("Demo table", "this a sample table with all features")
                ...
                .addActionElement(
                    dataTable ->
                        Icons.filter_menu_outline()
                            .clickable()
                            .addClickListener(
                                evt ->
                                    contactColumnHeaderFilterPlugin
    ...
    element.appendChild(
        Card.create(
                "SORT PLUGIN & SEARCH HEADER ACTION",
                "Allows the table to fire events useful for datasource to sort and search the data")
            .setCollapsible(true)
            .appendChild(table)
            .element());
