...
public class BanksComponent {
  ...
  public BanksComponent() {
    banksSelect = Select.<Bank>create("Bank").appendChild(PrefixAddOn.of(Icons.bank()));
    branchesSelect =
        Select.<Branch>create("Branch").appendChild(PrefixAddOn.of(Icons.domain())).disable();

    banksSelect.addChangeListener(
        (oldValue, bank) -> {
          branchesSelect.enable();
          List<Branch> branches = bank.getBranches();
          branchesSelect.removeAllOptions();
          for (Branch branch : branches) {
            branchesSelect.appendChild(
                SelectOption.create(branch.getName(), branch, branch.getName()));
          }
        });
  }
